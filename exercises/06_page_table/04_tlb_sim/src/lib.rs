...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        if let Some(ppn) = Tlb::lookup(&mut self.tlb , vpn, self.current_asid){
            return Some(ppn);
        }
        for &(mapping_asid, ref mapping) in &self.page_table {
            if mapping_asid == self.current_asid && mapping.vpn == vpn {
