...
pub fn alloc_stack() -> (Vec<u8>, usize) {
    ...
    buf.reserve(STACK_SIZE);
    let bottum = buf.as_mut_ptr() as usize;
    //let mut top = bottum + STACK_SIZE;
    let mut top = (bottum + STACK_SIZE) & !15;
    (buf , top)
} 
...
//cargo test -p stack_coroutine --target riscv64gc-unknown-linux-gnu