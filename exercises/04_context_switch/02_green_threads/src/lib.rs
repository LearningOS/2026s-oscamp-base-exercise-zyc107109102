...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        let bottum = stack.as_mut_ptr() as usize;
        let top = bottum + STACK_SIZE;
        let aligned = (top - 16) & !15;

        let mut ctx = TaskContext::default();

        ctx.ra = thread_wrapper as usize as u64;
        ctx.sp = aligned as u64;
    ...
    fn schedule_next(&mut self) {
        ...
        while attempts < num_threads {
            if self.threads[next_idx].state == ThreadState::Ready {
                ...
                let old_ctx_ptr = self.threads[current_idx].ctx.as_mut_ptr();
    ...
    if current_state != ThreadState::Finished {
        self.threads[current_idx].state = current_state;
